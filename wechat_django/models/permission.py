...
permission_required = {
    "material": ("article", "wechat_django.delete_material"),
    "article": ("wechat_django.delete_article",),
    "menu": ("wechat_django.delete_menu",),
    "messagehandler": (
...
def list_perm_names(app):
    ...
    rv.update(get_perm_name(app, permission) for permission in permissions)
    return rv


def get_perm_name(app, permission=""):
    if not permission:
        return "{prefix}{appname}".format(prefix=WECHATPERM_PREFIX, appname=app.name)
    return "{prefix}{appname}|{permission}".format(
        prefix=WECHATPERM_PREFIX, appname=app.name, permission=permission
    )
...
def get_perm_desc(perm_name, app):
    ...
    desc = permissions[permission] if permission else _("Can full control %(appname)s")
...
def get_require_perm_names(appname, permission=None):
    ...
    rv = set()
    perms = (
        permission_required.get(permission, [])
        if permission
...
def get_perms_by_codenames(codenames):
    return (
        Permission.objects.filter(content_type__app_label__startswith="wechat_django")
...
def delete_app_perms(sender, instance, *args, **kwargs):
    ...
    Permission.objects.filter(
        content_type=content_type, codename__in=list_perm_names(instance)
    ).delete()
...
def add_required_perms(sender, instance, action, *args, **kwargs):
    if action == "pre_add":
        ...
        for perm in perms:
            ...
            if appname:
                perm_names.update(get_require_perm_names(appname, permission))
        if perm_names:
...
def get_user_permissions(user, app=None, exclude_sub=False, exclude_manage=False):
    ...
    if exclude_sub:
        excludes.update(
            permission for permission in permissions if permission.find("_") != -1
        )
...
def match_permission(perm_name):
    ...
    pattern = str4format.format(
        label="wechat_django", prefix=WECHATPERM_PREFIX
    ).replace("|", "[|]")
    ...
    with transaction.atomic():
        ...
        for app in apps:
            for perm_name in get_perm_names(app, permissions):
                query = dict(codename=perm_name, content_type=content_type)
                defaults = query.copy()
                defaults.update(dict(name=get_perm_desc(perm_name, app)))
                Permission.objects.update_or_create(defaults=defaults, **query)
...
def downgrade_perms(permissions):
    ...
    with transaction.atomic():
        ...
        for app in apps:
            Permission.objects.filter(
                content_type=content_type, codename__in=get_perm_names(app, permissions)
            ).delete()
