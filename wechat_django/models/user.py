...
class WeChatUserManager(m.Manager):
    def upsert_by_dict(self, user_dict, app=None):
        ...
        updates = {k: v for k, v in user_dict.items() if k in model_fields(self.model)}
        updates["synced_at"] = tz.now()
        return self.update_or_create(
            defaults=updates, app=app, openid=updates["openid"]
        )[0]
...
class WeChatUser(WeChatModel):
    ...
    app = m.ForeignKey(
        WeChatApp, related_name="users", null=False, editable=False, on_delete=m.CASCADE
    )
    ...
    sex = m.SmallIntegerField(_("gender"), choices=enum2choices(Gender), null=True)
    ...
    remark = m.CharField(_("WeChat remark"), max_length=30, blank=True, null=True)
        ...
        try:
            ...
            if sync_user:
                try:
                    ...
                    if (
                        isinstance(e, WeChatClientException)
        ...
        if detail:
            ...
            with transaction.atomic():
                return list(
                    ...
                    for o in map(lambda openid: dict(app=app, openid=openid), openids)
                )
    ...
    def fetch_users(cls, app, openids):
        ...
        update_dicts = map(
            lambda o: {k: v for k, v in o.items() if k in fields}, user_dicts
        )
        rv = []
        with transaction.atomic():
            ...
            for user_dict in user_dicts:
                ...
                user = cls.objects.update_or_create(
                    defaults=defaults, app=app, openid=defaults["openid"]
                )[0]
    ...
    def upsert_by_oauth(cls, app, user_dict):
        """根据oauth的结果更新"""
        assert "openid" in user_dict, "openid not found"
        updates = {k: v for k, v in user_dict.items() if k in model_fields(cls)}
        updates["synced_at"] = tz.now()
