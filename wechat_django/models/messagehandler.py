...
class MessageHandlerManager(models.Manager):
...
class MessageHandler(WeChatModel):
    ...
    src = models.PositiveSmallIntegerField(
        choices=((Source.MP, "wechat"), (Source.SELF, "self"), (Source.MENU, "menu")),
        default=Source.SELF,
        editable=False,
    )
    strategy = models.CharField(
        _("strategy"),
        max_length=10,
        choices=enum2choices(ReplyStrategy),
        default=ReplyStrategy.REPLYALL,
    )

    flags = models.IntegerField(_("flags"), default=False)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["app", "weight", "created_at"]),
        ]
    ...
    def matches(message_info):
        ...
        """
        handlers = message_info.app.message_handlers.prefetch_related("rules").all()
        ...
        with transaction.atomic():
            ...
            if subscribe_reply:
                ...
                handler = app.message_handlers.create_handler(
                    ...
                    created_at=timezone.datetime.fromtimestamp(0),
                    rules=[Rule(type=Rule.Type.EVENT, event=cls.EventType.SUBSCRIBE)],
    ...
    def from_mp(cls, app, handler, src=None):
        ...
        return app.message_handlers.create_handler(
            ...
            rules=[Rule.from_mp(rule) for rule in handler["keyword_list_info"][::-1]],
            replies=[
    ...
    def handlerlog(cls, request):
        ...
        args = dict(params=request.GET, body=request.body, ip=get_ip(request))
        s = "%s - {0}".format(args)
