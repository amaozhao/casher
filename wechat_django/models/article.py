...
class Article(WeChatModel):
    material = m.ForeignKey(Material, related_name="articles", on_delete=m.CASCADE)
    ...
    author = m.CharField(_("author"), max_length=24, blank=True, null=True, default="")
    digest = m.CharField(_("digest"), max_length=256, blank=True, null=True, default="")
    ...
    need_open_comment = m.BooleanField(_("need open comment"), null=True, default=None)
    ...
    def thumb_url(self):
        if self._thumb_url is None and self.thumb_media_id:
            ...
            image = self.app.materials.filter(media_id=self.thumb_media_id).first()
    ...
    def _migrate_news(cls, app, src, news):
        ...
        return app.upload_material(
            [article.to_json() for article in articles], type=Material.Type.NEWS
        )
