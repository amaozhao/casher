...
class Reply(WeChatModel):
    ...
    handler = m.ForeignKey(MessageHandler, related_name="replies", on_delete=m.CASCADE)

    type = m.CharField(
        _("type"), db_column="type", max_length=16, choices=enum2choices(MsgType)
    )
    ...
    def reply_custom(self, message_info):
        ...
        try:
            ...
            if not hasattr(func, "message_handler"):
                ...
                hasattr(func.message_handler, "__contains__")
    ...
    def normal_reply(self, message):
        ...
        if self.type == self.MsgType.NEWS:
            ...
            # 将media_id转为content
            data = dict(articles=media.articles_json, media_id=self.content["media_id"])
    ...
    def from_mp(cls, app, data, src=None):
        ...
        if type == "img":
            ...
            data = dict(
                content='<a href="{0}">{1}</a>'.format(data["content"], _("video"))
            )
        ...
        if type == cls.MsgType.TEXT:
            ...
            media_id = app.as_permenant_material(data["content"], src=src, save=False)
            ...
            kwargs.update(media_id=media_id, content=data["news_info"]["list"])
        else:
    ...
    def from_menu(cls, menu, data, src=None):
        ...
        if type == cls.MsgType.TEXT:
            ...
            media_id = (
                app.as_permenant_material(data["value"], save=False, src=src)
                if app
                else data["value"]
            )
            kwargs.update(media_id=media_id)
