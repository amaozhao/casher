...
class MessageLog(WeChatModel):
    ...
    msg_id = models.BigIntegerField(_("msgid"), null=True)
    type = models.CharField(
        _("message type"), max_length=24, choices=enum2choices(Rule.ReceiveMsgType)
    )
    content = JSONField(_("content"))
    direct = models.BooleanField(_("direct"), default=Direct.USER2APP)

    raw = models.TextField(null=True, blank=True, default=None)
    ...
    class Meta(object):
        ...
        indexes = [models.Index(fields=["app", "created_at"])]
    ...
    def _from_message(cls, message, app, user, raw=""):
        ...
        if message.time:
            kwargs["created_at"] = timezone.datetime.fromtimestamp(message.time)
    ...
    def from_reply(cls, reply, app, user):
        ...
        if reply.time:
            kwargs["created_at"] = timezone.datetime.fromtimestamp(reply.time)
        return cls.objects.create(**kwargs)

    def __str__(self):
        return _("%(type)s消息: %(msg_id)s") % dict(type=self.type, msg_id=self.msg_id)
