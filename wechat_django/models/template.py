...
class Template(WeChatModel):
    app = m.ForeignKey(WeChatApp, related_name="templates", on_delete=m.CASCADE)
    ...
    def sync(cls, app):
        ...
        with transaction.atomic():
            (
                app.templates.exclude(
                    template_id__in=[t["template_id"] for t in templates]
                ).delete()
            )
            rv = []
            for t in templates:
                defaults = dict(app=app)
                defaults.update({k: v for k, v in t.items() if k in model_fields(cls)})
                template = app.templates.update_or_create(
                    app=app, template_id=t["template_id"], defaults=defaults
                )[0]
                rv.append(template)
            return rv
    ...
    def _send_service(self, openid, data, url=None, appid=None, pagepath=None):
        """发送服务号模板消息"""
        miniprogram = dict(appid=appid, pagepath=pagepath) if appid else None
