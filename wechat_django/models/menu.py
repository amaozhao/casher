...
class MenuManager(m.Manager):
    def get_queryset(self):
        return super(MenuManager, self).get_queryset().prefetch_related("sub_button")
...
class Menu(WeChatModel):
    ...
    app = m.ForeignKey(WeChatApp, related_name="menus", on_delete=m.CASCADE)
    name = m.CharField(_("name"), max_length=16)
    menuid = m.IntegerField(_("menuid"), null=True, blank=True)
    parent = m.ForeignKey(
        "Menu", related_name="sub_button", null=True, blank=True, on_delete=m.CASCADE
    )
    type = m.CharField(
        _("type"), max_length=20, choices=enum2choices(Event), null=True, blank=True
    )
    content = JSONField()
    ...
    def json2menu(cls, data, app, src=None):
        ...
        if not menu.type:
            ...
            menu.sub_button.add(
                *[
                    cls.json2menu(sub, app, src)
                    for sub in (data.get("sub_button") or dict(list=[])).get("list")
                ]
            )
        elif menu.type in (cls.Event.VIEW, cls.Event.CLICK, cls.Event.MINIPROGRAM):
            menu.content = data
        else:
            # 要当作回复处理了
    ...
    def to_json(self):
        ...
        if self.type:
            rv["type"] = self.type
            if self.type in (Menu.Event.CLICK, Menu.Event.VIEW, Menu.Event.MINIPROGRAM):
