...
class UserTag(WeChatModel):
    ...
    id = models.IntegerField(_("tag id"))
    name = models.CharField(_("tag name"), max_length=30)
    users = models.ManyToManyField(WeChatUser, related_name="tags")
    ...
    class Meta(object):
        ...
        indexes = [models.Index(fields=["app", "name"])]
        ordering = ("app", "id")
    ...
    def sync(cls, app):
        ...
        with transaction.atomic():
            ...
            # 处理已经移除的tags
            deleted_tag_ids = db_tag_ids.difference({tag["id"] for tag in tags})
            ...
            for tag in tags:
                ...
                if tag["id"] not in db_tag_ids:
                    ...
                    db_tag = app.user_tags.filter(id=tag["id"]).update(name=tag["name"])
                rv.append(db_tag)
            # 存在一个问题
...
@receiver(models.signals.m2m_changed, sender=UserTag.users.through)
...
@receiver(models.signals.m2m_changed, sender=WeChatUser.tags.through)
