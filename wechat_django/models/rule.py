...
class Rule(WeChatModel):
    ...
    type = m.CharField(_("type"), max_length=16, choices=enum2choices(Type))  # 规则类型
    ...
    def _custom_match(self, message_info):
        ...
        try:
            ...
            if not hasattr(func, "message_rule"):
                ...
                hasattr(func.message_rule, "__contains__")
    ...
    def _match(self, message):
        ...
        if self.type == self.Type.ALL:
            ...
            return message.type == self.ReceiveMsgType.EVENT and self._event_match(
                message
            )
        elif self.type == self.Type.EVENTKEY:
            return (
                message.type == self.ReceiveMsgType.EVENT
            ...
            return (
                message.type == self.ReceiveMsgType.TEXT
            ...
            return message.type == self.ReceiveMsgType.TEXT and re.search(
                self.content["pattern"], message.content
            )
        return False
    ...
    def _event_match(self, message):
        ...
        if target == MessageHandler.EventType.SUBSCRIBE:
            ...
            return event in (MessageHandler.EventType.SUBSCRIBE, "subscribe_scan")
        else:
            return event == target

    @classmethod
    def from_mp(cls, data, handler=None):
        return cls(handler=handler, type=data["match_mode"], pattern=data["content"])
