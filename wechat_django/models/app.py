...
class WeChatApp(m.Model):
    ...
    type = m.PositiveSmallIntegerField(
        _("type"), default=Type.SERVICEAPP, choices=enum2choices(Type)
    )
    ...
    def crypto(self):
        ...
        if not hasattr(self, "_crypto"):
            self._crypto = WeChatCrypto(self.token, self.encoding_aes_key, self.appid)
        return self._crypto
    ...
    def build_url(self, urlname, kwargs=None, request=None, absolute=False):
        ...
        # location = reverse(f"wechat_django:{urlname}", kwargs=kwargs)
        location = reverse(f"{urlname}", kwargs={})
        if not absolute:
            return location

        if request and not self.site_host:
            baseurl = "{}://{}".format(
                "https" if self.site_https else request.scheme, request.get_host()
            )
        else:
    ...
    def logger(self, name):
        return logging.getLogger("wechat.{name}.{app}".format(name=name, app=self.name))
