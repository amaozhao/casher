...
class MaterialManager(m.Manager):
    def create_material(self, **kwargs):
        ...
        if type == Material.Type.NEWS:
            ...
            kwargs = {
                key: kwargs[key] for key in model_fields(self.model) if key in kwargs
            }
    ...
    def create_news(self, **kwargs):
        ...
        record = dict(type=Material.Type.NEWS, update_time=kwargs["update_time"])
        record.update(query)
...
class Material(WeChatModel):
    ...
    app = m.ForeignKey(WeChatApp, related_name="materials", on_delete=m.CASCADE)

    type = m.CharField(_("type"), max_length=5, choices=(enum2choices(Type)))
    ...
    update_time = m.IntegerField(_("update time"), editable=False, null=True)
    ...
    def sync(cls, app, id=None, type=None):
        ...
        if id:
            ...
            return app.materials.create_material(type=type, media_id=id, **data)
        else:
            updated = []
    ...
    def sync_type(cls, app, type):
        ...
        return [app.materials.create_material(type=type, **item) for item in updates]
    ...
    def get_all(cls, app, type):
        ...
        while True:
            data = app.client.material.batchget(
                media_type=type, offset=offset, count=count
            )
            rv.extend(data["item"])
    ...
    def articles_json(self):
        return list(
            map(
                lambda o: dict(
                    title=o.title, description=o.digest, image=o.thumb_url, url=o.url
                ),
                self.articles.all(),
            )
        )
    ...
    def get_response_file_info(resp, type=None):
        ...
        return dict(filename=filename, ext=ext, type=type, content_type=content_type)
