...
class WeChatViewMixin(object):
    ...
    def initialize_request(self, request, *args, **kwargs):
        ...
        if not hasattr(request, "wechat"):
            request.wechat = self._create_wechat_info(request, *args, **kwargs)
        request.wechat = self._update_wechat_info(request, *args, **kwargs)
        return request
    ...
    def as_view(cls, **initKwargs):
        ...
        initKwargs["app_queryset"] = (
            initKwargs.get("app_queryset") or cls.app_queryset or WeChatApp.objects
        )
    ...
    def _create_wechat_info(self, request, *args, **kwargs):
        ...
        wechat = WeChatInfo(
            _request=request, _appname=appname, _app_queryset=self.app_queryset
        )
