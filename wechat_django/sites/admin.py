...
class WeChatAdminSiteMixin(CustomObjectToolAdminSiteMixin):
    ...
    def _build_wechat_app_dict(self, request):
        ...
        if not request.user.is_superuser:
            ...
            allowed_apps = {k for k, ps in perms.items() if ps != {"manage"}}
        ...
        app_perms = [
            dict(
                ...
                admin_url=reverse(
                    ...
                    kwargs=dict(wechat_app_id=app.id, app_label=app_label),
                ),
            )
            for app in apps
        ]
    ...
    def _build_wechat_func_dict(self, request):
        ...
        rv = dict(
            ...
            app_url=reverse(
                "admin:wechat_funcs_list",
                current_app=self.name,
                kwargs=dict(wechat_app_id=app_id, app_label=app_label),
            ),
    ...
    def wechat_index(self, request, *args, **kwargs):
        """某个公众号后台管理首页"""
        return super(WeChatAdminSiteMixin, self).app_index(request, *args, **kwargs)
