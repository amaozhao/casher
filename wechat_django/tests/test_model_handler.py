...
class HandlerTestCase(WeChatTestCase):
    def test_available(self):
        ...
        handler_available = self._create_handler(rule, starts=now - day, ends=now + day)
    ...
    def test_subscribe_events(self, _get_appname):
        ...
        url = reverse("wechat_django:handler", kwargs=dict(appname=self.app.name))
        ...
        request = self.rf().post(url, subscribe_event_text, content_type="text/xml")
        ...
        request = self.rf().post(url, unsubscribe_event_text, content_type="text/xml")
        request = handler.initialize_request(request)
        self.assertEqual(handler.post(request, self.app.name), "")
        user.refresh_from_db()
