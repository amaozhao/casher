...
class UserTestCase(WeChatTestCase):
    ...
    def test_miniprogram_messages(self):
        ...
        session = Session(
            user=user, type=Session.Type.MINIPROGRAM, auth=dict(session_key=session_key)
        )
        ...
        sign = "fake_sign"
        self.assertRaises(
            InvalidSignatureException, session.validate_message, raw_data, sign
        )
        ...
        self.assertRaises(
            (TypeError, ValueError), session.decrypt_message, fake_encrypted_data, iv
        )
        fake_iv = "abc"
        self.assertRaises(
            (TypeError, ValueError), session.decrypt_message, encrypted_data, fake_iv
        )
        exipred_session_key = "abc"
        session.auth["session_key"] = exipred_session_key
        self.assertRaises(
            (TypeError, ValueError), session.decrypt_message, encrypted_data, iv
        )
