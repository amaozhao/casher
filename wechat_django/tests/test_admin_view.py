...
class AdminViewTestCase(WeChatTestCase):
    ...
    def test_indexes(self):
        ...
        self.assertRequestSuccess(
            reverse("admin:app_list", kwargs=dict(app_label="wechat_django"))
        )
        self.assertRequestSuccess(
            reverse(
                "admin:wechat_funcs_list",
                kwargs=dict(app_label="wechat_django", wechat_app_id=self.app.id),
            )
        )
    ...
    def test_article_view(self):
        ...
        self.assertModelViewSuccess(
            models.Article,
            excludes=("history", "delete", "change"),
            status={"add": 403},
        )

        material = models.Material.objects.create(app=self.app, media_id="media_id")
        article = models.Article.objects.create(
            material=material, content="", url="url", index=0
        )
        self.assertModelViewSuccess(
    ...
    def test_material_view(self):
        ...
        material = models.Material.objects.create(
            app=self.app, media_id="media_image", type=models.Material.Type.IMAGE
        )
        self.assertModelViewSuccess(models.Material, material.id, status={"add": 403})

        material = models.Material.objects.create(
            app=self.app, media_id="media_video", type=models.Material.Type.VIDEO
        )
        self.assertModelViewSuccess(models.Material, material.id, status={"add": 403})

        material = models.Material.objects.create(
            app=self.app, media_id="media_voice", type=models.Material.Type.VOICE
        )
        self.assertModelViewSuccess(models.Material, material.id, status={"add": 403})

        material = models.Material.objects.create(app=self.app, media_id="media_new")
        article = models.Article.objects.create(
            material=material, content="", url="url", index=0
        )
        self.assertModelViewSuccess(models.Material, material.id, status={"add": 403})
    ...
    def test_messagehandler_view(self):
        ...
        handler = models.MessageHandler.objects.create_handler(
            rules=[models.Rule(type=models.Rule.Type.ALL)],
            replies=[models.Reply(type=models.Reply.MsgType.TEXT, pattern="test")],
            app=self.app,
        )
    ...
    def test_messagelog_view(self):
        ...
        log = models.MessageLog.objects.create(
            app=self.app, user=user, type=models.Rule.ReceiveMsgType.TEXT, content={}
        )
        self.assertModelViewSuccess(models.MessageLog, log.id, status={"add": 403})
    ...
    def assertModelViewSuccess(
        self, model, id=None, excludes=None, status=None, wechat=True
    ):
        ...
        "changelist" not in excludes and self.assertRequestSuccess(
            make_url("changelist"), status_code=status.get("changelist")
        )
        "add" not in excludes and self.assertRequestSuccess(
            make_url("add"), status_code=status.get("add")
        )
        # self.assertRequestSuccess(make_url("autocomplete"))
        "history" not in excludes and self.assertRequestSuccess(
            make_url("history", object_id=id), status_code=status.get("history")
        )
        "delete" not in excludes and self.assertRequestSuccess(
            make_url("delete", object_id=id), status_code=status.get("delete")
        )
        "change" not in excludes and self.assertRequestSuccess(
            make_url("change", object_id=id), status_code=status.get("change")
        )
