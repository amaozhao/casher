...
class HandlerTestCase(WeChatTestCase):
    def setUp(self):
        ...
        MessageHandler.objects.create_handler(
            app=self.app,
            rules=[Rule(type=Rule.Type.EQUAL, pattern=self.match_str)],
            replies=[Reply(type=Reply.MsgType.TEXT, content=self.success_reply)],
        )
    ...
    def test_badrequest(self):
        ...
        # 错误时间戳
        query = dict(timestamp=str(int(time.time() - 1000)), nonce=nonce)
    ...
    def test_echostr(self):
        ...
        timestamp = str(int(time.time()))
        nonce = "123456"
        query = dict(timestamp=timestamp, nonce=nonce, echostr=echostr)
    ...
    def sign(self, query):
        signer = WeChatSigner()
        signer.add_data(self.app.token, query["timestamp"], query["nonce"])
    ...
    def post(self, query, content=""):
        ...
        if "signature" not in query:
            query["signature"] = self.sign(query)
        return self.client.generic("POST", self.url + "?" + urlencode(query), xml)
    ...
    @property
    def url(self):
        return reverse("wechat_django:handler", kwargs=dict(appname=self.app.name))
