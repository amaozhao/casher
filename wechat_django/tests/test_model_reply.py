...
class ReplyTestCase(WeChatTestCase):
    def test_reply(self):
        """测试一般回复"""
        sender = "openid"
        message = messages.TextMessage(dict(FromUserName=sender, content="xyz"))

        def _test_reply(type, **kwargs):
            reply = Reply(
                type=type, **{k: v for k, v in kwargs.items() if v is not None}
            )
        ...
        description = "desc"
        reply = _test_reply(
            Reply.MsgType.VIDEO, media_id=media_id, title=title, description=description
        )
    ...
    def test_multireply(self):
        ...
        reply2 = "def"
        replies = [
            dict(type=Reply.MsgType.TEXT, content=reply1),
            dict(type=Reply.MsgType.TEXT, content=reply2),
        ]
        ...
        with wechatapi_accesstoken(), wechatapi(
            api, dict(errcode=0, errmsg=""), callback
        ):
    ...
    def test_custom(self):
        ...
        handler_fail = _get_handler(
            "app_only_handler", WeChatApp.objects.get_by_name("test1")
        )
    ...
    def test_send(self):
        ...
        reply = Reply(
            type=msg_type, media_id=media_id, title=title, description=description
        ).reply(message)
        funcname, kwargs = Reply.reply2send(reply)
        self.assertTrue(hasattr(client, funcname))
        ...
        # 确认消息发送
        handler = self._create_handler(
            replies=dict(type=Reply.MsgType.TEXT, content=content)
        )
        ...
        with wechatapi_accesstoken(), wechatapi(
            "/cgi-bin/message/custom/send", dict(errcode=0), callback
        ):
            handler.replies.all()[0].send(message)

    def _wrap_message(self, message):
        return WeChatMessageInfo(_app=self.app, _message=message)
