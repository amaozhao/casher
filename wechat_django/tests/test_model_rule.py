...
class HandlerTestCase(WeChatTestCase):
    def test_match(self):
        ...
        another_content = "abc"
        another_text_message = _create_msg(
            messages.TextMessage, content=another_content
        )
        ...
        # 测试类型匹配
        rule = Rule(type=Rule.Type.MSGTYPE, msg_type=Rule.ReceiveMsgType.IMAGE)
        self.assertNotMatch(rule, text_message)
        self.assertMatch(rule, image_message)

        # 测试事件匹配
        rule = Rule(type=Rule.Type.EVENT, event=MessageHandler.EventType.SUBSCRIBE)
        self.assertNotMatch(rule, text_message)
        ...
        # 测试指定事件匹配
        rule = Rule(
            type=Rule.Type.EVENTKEY, event=MessageHandler.EventType.CLICK, key=event_key
        )
        self.assertNotMatch(rule, text_message)
        ...
        self.assertTrue(rule.match(message_info))
        self.assertFalse(rule.match(self._msg2info(text_message, another_app)))
        ...
        handler3 = self._create_handler(
            rules=[
                dict(type=Rule.Type.EQUAL, pattern=content),
                dict(type=Rule.Type.EQUAL, pattern=another_content),
            ],
            name="3",
        )
        self.assertTrue(handler3.is_match(self._msg2info(text_message)))
        self.assertTrue(handler3.is_match(self._msg2info(another_text_message)))
        self.assertFalse(handler3.is_match(self._msg2info(click_event)))
        ...
        handler2 = self._create_handler(
            rules=[dict(type=Rule.Type.EQUAL, pattern=content)], name="2"
        )
        handler4 = self._create_handler(
            rules=[dict(type=Rule.Type.EVENT, event=MessageHandler.EventType.CLICK)],
            name="4",
            weight=-5,
        )
