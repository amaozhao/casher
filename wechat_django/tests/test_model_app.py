...
class AppTestCase(WeChatTestCase):
    ...
    def test_custom_accesstoken_url(self):
        ...
        with mock.patch.object(_Client, "_fetch_access_token"):
            ...
            self.assertEqual(_Client._fetch_access_token.call_args[0][0], new_url)
    ...
    def test_miniprogram_auth(self):
        ...
        with mock.patch.object(WeChatWxa, "code_to_session"):
            return_value = dict(openid=openid, session_key=session_key, unionid=unionid)
    ...
    def test_build_url(self):
        ...
        def assertUrlCorrect(
            hostname, urlname, request=None, secure=False, kwargs=None
        ):
            kwargs_copy = (kwargs or dict()).copy()
            kwargs_copy["appname"] = self.app.name
            location = reverse("wechat_django:{0}".format(urlname), kwargs=kwargs_copy)
            self.assertEqual(
            ...
            self.assertEqual(
                ...
                self.app.build_url(urlname, kwargs, request=request, absolute=True),
            )
