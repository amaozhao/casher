...
class UserTestCase(WeChatTestCase):
    def test_sync(self):
        ...
        def assertTagSyncSuccess(custom_tags):
            with mock.patch.object(WeChatGroup, "get"), mock.patch.object(
                WeChatTag, "update"
            ), mock.patch.object(WeChatTag, "create"):
                remote_tags = self.base_tags[:] + [
                    dict(id=id, name=name, count=0) for id, name in custom_tags.items()
                ]
    ...
    def test_sync_users(self):
        """测试同步标签下的用户"""
        with mock.patch.object(WeChatTag, "iter_tag_users"), mock.patch.object(
            WeChatUser, "upsert_users"
        ):
    ...
    def test_change_user_tags(self):
        ...
        with mock.patch.object(WeChatTag, "tag_user"), mock.patch.object(
            WeChatTag, "untag_user"
        ):
            ...
            self.assertEqual(WeChatTag.tag_user.call_args, ((tag2.id, user.openid),))
            ...
            self.assertEqual(WeChatTag.untag_user.call_args, ((tag1.id, user.openid),))
            ...
            self.assertRaises(
                UserTag.DoesNotExist, lambda: user.tags.get(app=self.app, id=tag1.id)
            )
    ...
    def test_tag_users(self):
        ...
        tag = UserTag.objects.create(app=self.app, id=101, name="tag1", _tag_local=True)
        del tag._tag_local
        ...
        with mock.patch.object(WeChatTag, "tag_user"), mock.patch.object(
            ...
            self.assertEqual(WeChatTag.tag_user.call_args, ((tag.id, [user2.openid]),))
    ...
    def test_edit_tag(self):
        ...
        with mock.patch.object(WeChatTag, "create"):
            WeChatTag.create.return_value = dict(id=id, name=name)
        ...
        with mock.patch.object(WeChatTag, "delete"):
            tag.delete()
            self.assertRaises(
                UserTag.DoesNotExist, lambda: UserTag.objects.get(app=self.app, id=id)
            )
