...
class AdminBaseTestCase(WeChatTestCase):
    ...
    def test_get_queryset(self):
        ...
        self.assertEqual(queryset.filter(openid=openid).count(), 1)
        self.assertEqual(queryset.count(), queryset.filter(app=self.app).count())
        self.assertEqual(queryset.exclude(app=self.app).count(), 0)
