...
class WeChatTestCaseBase(TestCase):
    def assertCallArgsEqual(self, func, args=(), kwargs=None):
        ...
        self.assertEqual({k: v for k, v in call_kwargs.items() if k in kwargs}, kwargs)

    def load_data(self, path):
        res_file = os.path.join(os.path.dirname(__file__), "data", "%s.json" % path)
...
class WeChatTestCase(WeChatTestCaseBase):
    ...
    def setUpTestData(cls):
        ...
        WeChatApp.objects.create(
            title="test", name="test", appid="appid", appsecret="secret", token="token"
        )
        WeChatApp.objects.create(
            title="test1",
            name="test1",
    ...
    def _create_handler(self, rules=None, name="", replies=None, app=None, **kwargs):
        ...
        replies = [Reply(**reply) for reply in replies]

        app = app or self.app
        return MessageHandler.objects.create_handler(
            app=app, name=name, rules=rules, replies=replies, **kwargs
        )
