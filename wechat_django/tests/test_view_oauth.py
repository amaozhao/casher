...
def oauth_api(openid, callback=None):
    kwargs = dict(
        netloc=r"(.*\.)?api\.weixin\.qq\.com$", path="/sns/oauth2/access_token"
    )

    def mock(url, request):
        headers = {"Content-Type": "application/json"}
        data = dict(
            access_token="123", refresh_token="123", expires_in=7200, openid=openid
        )
...
def snsinfo_api(openid, callback=None, **data):
    kwargs = dict(netloc=r"(.*\.)?api\.weixin\.qq\.com$", path="/sns/userinfo")
...
def test_oauth_view(request, *args):
    ...
    return dict(openid=user.openid, id=user.id, args=args)


urlpatterns = [url(r"^test/(.+)$", test_oauth_view)]
...
class OAuthTestCase(WeChatTestCase):
    ...
    def test_redirect_uri(self):
        ...
        view_cls = WeChatOAuthView(appname=self.app.name, redirect_uri=redirect_uri)
        request = rf.get(api)
        ...
        view_cls.setup(request)
        request = view_cls.initialize_request(request)
        self.assertEqual(request.wechat.redirect_uri, absolute_uri(redirect_uri))
        ...
        view_cls = WeChatOAuthView(
            appname=self.app.name, redirect_uri=callable_redirect
        )
        request = rf.get(redirect_uri)
        SessionMiddleware().process_request(request)
        view_cls.setup(request)
    ...
    def test_unauthorization(self):
        ...
        self.assertEqual(
            resp.url, client.authorize_url(redirect_uri, WeChatSNSScope.USERINFO, state)
        )
        ...
        client = WeChatOAuthClient(self.app)
        self.assertEqual(
            resp.url, client.authorize_url(redirect_uri, WeChatSNSScope.USERINFO, path)
        )
    ...
    def test_request(self):
        ...
        # 设置了response
        resp = response.HttpResponseForbidden()
        handler = self._make_handler(request, redirect_uri=redirect_uri, response=resp)
    ...
    def test_decorator(self):
        ...
        user = WeChatUser.objects.upsert_by_dict(dict(openid=openid), app=self.app)
        ...
        # 测试scope
        wrapped_view = wechat_auth(self.app.name, scope=WeChatSNSScope.USERINFO)(view)
        request = self._create_request("/")
        request.session[session_key] = openid
        ...
        wrapped_view = wechat_auth(self.app.name, redirect_uri=redirect_uri)(view)
        request = self._create_request("/")
        request.session[session_key] = openid
        ...
        # 测试state
        state = "abc"
        wrapped_view = wechat_auth(self.app.name, state=state)(view)
        request = self._create_request("/")
        request.session[session_key] = openid
        ...
        self.assertIsInstance(
            wrapped_view(request_post), response.HttpResponseNotAllowed
        )

        # 测试仅post
        wrapped_view = wechat_auth(self.app.name, methods=["POST"])(view)
        self.assertIsInstance(
            wrapped_view(request_get), response.HttpResponseNotAllowed
        )
    ...
    def test_view(self):
        ...
        with oauth_api(openid):
            resp = self.client.get(path + "?code=123", follow=True, HTTP_HOST=host)
