...
class Handler(WeChatView):
    ...
    def finalize_response(self, request, resp, *args, **kwargs):
        ...
        return super(Handler, self).finalize_response(request, resp, *args, **kwargs)
    ...
    def post(self, request, appname):
        ...
        signals.message_received.send(
            request.wechat.app.staticname, message_info=message_info
        )
        try:
            reply = self._handle(message_info)
            signals.message_handled.send(
                request.wechat.app.staticname, message_info=message_info, reply=reply
            )
            if reply:
                xml = reply.render()
            ...
            signals.message_error.send(
                request.wechat.app.staticname, message_info=message_info, exc=exc
            )
            raise
        return ""
