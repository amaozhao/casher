...
def anchor(text, href, **kwargs):
    ...
    def wrapper(modeladmin, obj):
        kwargs.update(href=href, text=text)
...
def foreignkey(field_name):
    ...
    def _linkify(obj):
        ...
        link_url = reverse(
            view_name, kwargs=dict(object_id=linked_obj.id, wechat_app_id=obj.app_id)
        )
...
def get_request_params(request, param):
    ...
    if not hasattr(request, param):
        ...
        value = request.GET.get(param) or preserved_filters.get(param)
