    ...
    def delete_selected_recusively(self, request, queryset):
        ...
        if not request.POST.get("post"):
            ...
            resp.context_data.update(
                wechat_app=request.app, wechat_app_id=request.app_id
            )
            return resp
...
class WeChatChangeList(ChangeList):
    ...
    def url_for_result(self, result):
        view = "admin:%s_%s_change" % (self.opts.app_label, self.opts.model_name)
        kwargs = dict(
...
class WeChatModelAdminMetaClass(forms.MediaDefiningClass):
    def __new__(cls, name, bases, attrs):
        ...
        self = super(WeChatModelAdminMetaClass, cls).__new__(cls, name, bases, attrs)
        if name != "WeChatModelAdmin" and model:
            registered_admins.append((model, self))
        return self


class WeChatModelAdmin(
    six.with_metaclass(WeChatModelAdminMetaClass, CustomObjectToolModelAdmin)
):
    """所有微信相关业务admin的基类
    ...
    def get_queryset(self, request):
        return (
            super(WeChatModelAdmin, self)
            .get_queryset(request)
            .filter(app_id=request.app_id)
        )
...
class DynamicChoiceForm(forms.ModelForm):
    ...
    def save(self, commit=True, *args, **kwargs):
        ...
        setattr(model, self.content_field, self.cleaned_data[self.content_field])
        if commit:
            model.save()
        return model
