...
class WeChatAppForm(forms.ModelForm):
    ...
    wechat_host = forms.CharField(
        label=_("WeChat host"), required=False, help_text=_("接收微信回调的域名")
    )
    wechat_https = forms.BooleanField(
        label=_("WeChat https"), required=False, help_text=_("回调地址是否为https")
    )

    accesstoken_url = forms.URLField(
        label=_("accesstoken url"),
        required=False,
    ...
    def save(self, commit=True):
        ...
        self.instance.configurations["SITE_HOST"] = self.cleaned_data.get(
            "wechat_host", ""
        )
        self.instance.configurations["SITE_HTTPS"] = self.cleaned_data.get(
            "wechat_https", None
        )
        self.instance.configurations["ACCESSTOKEN_URL"] = self.cleaned_data.get(
            "accesstoken_url", ""
        )
        self.instance.configurations["OAUTH_URL"] = self.cleaned_data.get(
            "oauth_url", ""
        )
...
class WeChatAppAdmin(admin.ModelAdmin):
    ...
    def callback(self, obj):
        return obj and obj.build_url("handler", request=self.request, absolute=True)
    ...
    def get_urls(self):
        ...
        if django.VERSION[0] < 2:
            for pattern in urlpatterns:
                pattern._regex = pattern._regex.replace("(.+)", "(?P<object_id>.+)")
        return urlpatterns
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if obj:
            rv = rv + ("name", "appid", "callback", "created_at", "updated_at")
            if obj.type != 0:
                rv = rv + ("type",)
        return rv
    ...
    def _get_allowed_apps(self, request):
        ...
        return set(
            appname for appname, permissions in perms.items() if "manage" in permissions
        )
    ...
    def get_deleted_objects(self, objs, request):
        deleted_objects, model_count, perms_needed, protected = super(
            WeChatAppAdmin, self
        ).get_deleted_objects(objs, request)

        from ..models import Template, WeChatUser

        ignored_models = [cls._meta.verbose_name for cls in (Template, WeChatUser)]
