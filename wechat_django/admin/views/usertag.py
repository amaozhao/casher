...
class UserTagAdmin(RecursiveDeleteActionMixin, WeChatModelAdmin):
    ...
    def sync_users(self, request, queryset, detail=True):
        ...
        return self._clientaction(request, action, _("Sync users failed with %(exc)s"))
    ...
    def count(self, obj):
        return '<a href="{link}">{count}</a>'.format(
            link="{0}?{1}".format(
                ...
                urlencode(dict(tags__in=obj._id)),
            ),
    ...
    def has_add_permission(self, request):
        return (
            super(UserTagAdmin, self).has_add_permission(request)
            and self.get_queryset(request).exclude(id__in=UserTag.SYS_TAGS).count()
            < 100
        )
