...
class MenuAdmin(WeChatModelAdmin):
    ...
    def detail(self, obj):
        ...
        if obj.type == Menu.Event.CLICK:
            ...
            rv = '<a href="{0}">{1}</a>'.format(obj.content.get("url"), _("link"))
    ...
    def operates(self, obj):
        del_url = reverse(
            "admin:wechat_django_menu_delete",
            kwargs=dict(object_id=obj.id, wechat_app_id=obj.app_id),
        )
        ...
        if not obj.parent and not obj.type and obj.sub_button.count() < 5:
            ...
            add_link = reverse(
                "admin:wechat_django_menu_add", kwargs=dict(wechat_app_id=obj.app_id)
            )
    ...
    def sync(self, request, obj=None):
        ...
        return self._clientaction(request, action, _("Sync menus failed with %(exc)s"))
