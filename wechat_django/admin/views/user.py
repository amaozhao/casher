...
class WeChatUserAdmin(WeChatModelAdmin):
    ...
    def subscribetime(self, obj):
        return obj.subscribe_time and timezone.datetime.fromtimestamp(
            obj.subscribe_time
        )
    ...
    def sync(self, request, obj=None, method="sync", kwargs=None):
        ...
        return self._clientaction(request, action, tpl, dict(method=_(method)))
    ...
    update = lambda self, request, queryset: self.sync(
        request, None, "fetch_users", dict(openids=[o.openid for o in queryset.all()])
    )
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        tags_field.queryset = tags_field.queryset.filter(app=app).exclude(
            id__in=UserTag.SYS_TAGS
        )
        return form

    def get_readonly_fields(self, request, obj=None):
        return tuple(
            o for o in self.fields if o not in ("alias", "remark", "tags", "comment")
        )

    def save_model(self, request, obj, form, change):
        if "remark" in form.changed_data:
            obj.app.client.user.update_remark(obj.openid, obj.remark)
        return super(WeChatUserAdmin, self).save_model(request, obj, form, change)
