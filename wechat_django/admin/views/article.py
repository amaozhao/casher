...
class ArticleAdmin(WeChatModelAdmin):
    ...
    source_url = anchor(_("source_url"), lambda self, obj: obj.content_source_url)
    ...
    def thumb_image(self, obj):
        return (
            obj.thumb_url
            and '<a href="{0}"><img width="200" src="{0}" /></a>'.format(obj.thumb_url)
        )
        ...
        if m:
            return '<a href="{link}" title="{title}">{title}</a>'.format(
                link=reverse(
                    ...
                    kwargs=dict(object_id=m.id, wechat_app_id=m.app_id),
                ),
                title=(
                    "{comment} ({media_id})".format(
                        comment=m.comment, media_id=m.media_id
                    )
                    if m.comment
                    else m.media_id
                ),
