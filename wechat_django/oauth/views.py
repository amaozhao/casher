...
class WeChatOAuthView(WeChatOAuthViewMixin, APIView):
    ...
    def prepare_init_kwargs(cls, **initKwargs):
        ...
        assert appname and isinstance(appname, six.text_type), "incorrect appname"

        response = initKwargs.get("response") or cls.response
        assert (
            response is None or callable(response) or isinstance(response, HttpResponse)
        ...
        for permission in self.get_permissions():
            if not permission.has_permission(request, self):
                if isinstance(permission, WeChatAuthenticated):
                    raise WeChatOAuthException(0, getattr(permission, "message", None))
                else:
