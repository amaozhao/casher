...
class WeChatPayForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (mch_cert or mch_key) and not (mch_cert and mch_key):
            self.add_error("_mch_cert", _("must upload both mch_cert and mch_key"))
        return rv
...
class WeChatAppWithPayAdmin(WeChatAppAdmin):
    ...
    def get_inline_instances(self, request, obj=None):
        rv = super(WeChatAppWithPayAdmin, self).get_inline_instances(request, obj)
        if (
            not obj
            or not obj.type
            & (AppType.SERVICEAPP | AppType.MINIPROGRAM | AppType.PAYPARTNER)
            or not has_wechat_permission(request, obj, "pay", "manage")
        ):
            rv = tuple(filter(lambda o: not isinstance(o, WeChatPayInline), rv))
        return rv
