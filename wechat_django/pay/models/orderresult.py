...
class UnifiedOrderResult(WeChatModel):
    ...
    order = m.OneToOneField(UnifiedOrder, on_delete=m.CASCADE, related_name="result")

    transaction_id = m.CharField(_("transaction_id"), max_length=32, null=True)
    ...
    settlement_total_fee = m.PositiveIntegerField(_("settlement_total_fee"), null=True)
    ...
    def update(self, result, signal=True, verify=True):
        ...
        if signal:
            order_updated.send(
                sender=self.order.pay.staticname,
                result=self,
                order=self.order,
