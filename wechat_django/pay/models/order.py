...
class UnifiedOrder(WeChatModel):
    ...
    fee_type = m.CharField(_("fee_type"), max_length=16, null=True, default=FeeType.CNY)
    total_fee = m.PositiveIntegerField(_("total_fee"))
    spbill_create_ip = m.CharField(
        _("spbill_create_ip"), max_length=64
    )  # TODO: 生成时是否应该赋值?
    ...
    def call_args(self, request=None, dt2py=False, **kwargs):
        ...
        self.spbill_create_ip = kwargs.pop(
            "client_ip", get_ip(request) or self.spbill_create_ip or get_external_ip()
        )
        if self._call_args:
    ...
    def jsapi_params(self, prepay_id, *args, **kwargs):
        return self.pay.client.jsapi.get_jsapi_params(prepay_id, *args, **kwargs)
    ...
    def sync(self):
        """更新订单状态"""
        result = self.pay.client.order.query(out_trade_no=self.out_trade_no)
        self.update(result)
    ...
    def update(self, result, signal=True, verify=True):
        ...
        try:
            ...
            obj = UnifiedOrderResult(order=self, transaction_id=transaction_id)
    ...
    def verify(self, result):
        ...
        # 检查out_order_no是否一致
        assert result["out_trade_no"] == self.out_trade_no, "incorrect out trade no"

        # 检查金额一致
        if result["trade_state"] == UnifiedOrderResult.State.SUCCESS:
            assert str(result["total_fee"]) == str(
                self.total_fee
            ), "incorrect total fee"
