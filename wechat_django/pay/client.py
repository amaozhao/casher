...
def load_cert(self):
    if self.pay.mch_cert and self.pay.mch_key:
        ...
        with NamedTemporaryFile("wb", delete=False) as mch_cert, NamedTemporaryFile(
            "wb", delete=False
        ) as mch_key:
            ...
            try:
                ...
                self.mch_key and os.path.exists(self.mch_key) and os.remove(
                    self.mch_key
                )
                self.mch_key = None
                self.mch_cert and os.path.exists(self.mch_cert) and os.remove(
                    self.mch_cert
                )
                self.mch_cert = None
    else:
    ...
    def _request(self, method, url_or_endpoint, **kwargs):
        ...
        with load_cert(self):
            try:
                ...
                log_args["err"] = e
                logger.warning(
                    "An error occurred when send wechat pay" "request: %s" % log_args,
                    exc_info=True,
                )
                raise
