...
class NotifyTestCase(WeChatPayTestCase):
    ...
    def test_response(self):
        """测试响应"""
        url = self.app.build_url("order_notify", kwargs=dict(payname=self.app.pay.name))
        # 测试成功响应
    ...
    def test_prepare_request(self):
        ...
        view = NotifyView()
        request = self.rf().post(url, data=xml, content_type="text/xml")
        ...
        # 测试签名错误
        self.assertRaises(
            WeChatPayNotifyError, view._prepare, request, self.app.pay.name
        )
    ...
    def test_notify_order(self):
        ...
        self.assertEqual(order.result.trade_state, UnifiedOrderResult.State.SUCCESS)

    @property
    def minimal_example(self):
        return dict(body="body", out_trade_no=str(uuid4()), total_fee=101)
