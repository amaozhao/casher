...
class OrderTestCase(WeChatPayTestCase):
    def test_create(self):
        ...
        order = self.app_sub.pay.create_order(user_sub, request, **self.minimal_example)
    ...
    def test_call_args(self):
        """测试调用参数"""
        key_map = dict(
            spbill_create_ip="client_ip", openid="user_id", sub_openid="sub_user_id"
        )
        full = self.full_example
        full["openid"] = "openid"
        # 将时间转换为字符串
        ...
        # 更新参数
        update = dict(client_ip="127.0.0.1", total_fee=200)
        full = self.full_example
        order = self.app.pay.create_order(request=request, **full)
